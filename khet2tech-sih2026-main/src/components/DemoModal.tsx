...
export default function DemoModal() {
  ...
  const [activeRouteMode, setActiveRouteMode] = useState<'mandi' | 'khet2tech'>('khet2tech');
  ...
  const khet2techTotalPayout = Math.round(netFarmpath * cropLot.quantityKg);
  const mandiCommission = baseline?.costBreakdown.intermediaryCostTotal || Math.round(mandiTotalPayout * 0.085);
  const khet2techCommission = optimal?.costBreakdown.intermediaryCostTotal || 0;
  const commissionSaved = Math.max(0, Math.round(mandiCommission - khet2techCommission));
  ...
  const khet2techSpoilageKg = optimal?.costBreakdown.expectedSpoilageKg || Math.round(cropLot.quantityKg * 0.032);
  const khet2techSpoilagePct = optimal?.costBreakdown.expectedSpoilagePct || 3.2;
  const rotKgSaved = Math.max(0, Math.round(mandiSpoilageKg - khet2techSpoilageKg));
  ...
  const chapters = [
    ...
    { id: 3, title: 'Khet2Tech Engine', tag: '3. Route' },
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-slate-900 text-white rounded-3xl shadow-2xl border border-slate-800 w-full max-w-4xl overflow-hidden flex flex-col max-h-[92vh]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-bold text-sm sm:text-base text-white">Khet2Tech Interactive Live Tour</h3>
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-5 bg-slate-900/60">
          ...
          {/* ================= CHAPTER 3: THE Khet2Tech ENGINE (INTERACTIVE ROUTE COMPARATOR) ================= */}
          {currentChapter === 3 && (
            <div className="space-y-4 animate-in fade-in duration-200">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <span className="text-[11px] font-bold text-emerald-400 uppercase tracking-wider">Step 3 • The Algorithmic Engine</span>
                  <h2 className="text-xl sm:text-2xl font-black text-white">How Khet2Tech Discovers the Route</h2>
                </div>
              ...
              <div className="flex items-center gap-2 p-1.5 bg-slate-950 rounded-2xl border border-slate-800">
                ...
                <button
                  onClick={() => setActiveRouteMode('khet2tech')}
                  className={`flex-1 py-2 rounded-xl text-xs font-bold transition-all ${
                    activeRouteMode === 'khet2tech'
                      ? 'bg-emerald-500 text-slate-950 shadow-md font-black'
                      : 'text-slate-400 hover:text-white'
                  }`}
                >
                  Option B: Khet2Tech Direct (Recommended)
                </button>
              </div>
                ...
                <div className="p-5 rounded-2xl bg-emerald-950/40 border border-emerald-500/30 space-y-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2 border-t border-emerald-800/40 text-xs">
                    ...
                    <div>
                      ...
                      <span className="font-bold text-emerald-400 font-mono">{khet2techSpoilagePct.toFixed(1)}% (Only {khet2techSpoilageKg.toLocaleString()} kg)</span>
                    </div>
                    <div>
          ...
          {currentChapter === 4 && (
            <div className="space-y-4 animate-in fade-in duration-200">
              ...
              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-3">
                ...
                <div className="space-y-1">
                  <div className="flex justify-between text-xs text-emerald-400 font-bold">
                    <span>Option B: Khet2Tech Intelligent Direct Route</span>
                    <span className="font-mono text-emerald-400">₹{khet2techTotalPayout.toLocaleString()} (+{gainPct}%)</span>
                  </div>
                    ...
                    {gainBreakdownTab === 'commission' && (
                      <>
                        ...
                        <p className="text-slate-200 text-xs leading-relaxed pt-1">
                          At conventional APMC Mandis, commission agents (Arhatiyas) automatically slice <strong>8.5% statutory commission</strong> plus loading and weighment deductions (totaling <strong>₹{mandiCommission.toLocaleString()}</strong>). Khet2Tech connects registered FPOs directly to the corporate factory gate under Section 40, completely reducing middleman commission to <strong>₹0.00</strong>.
                        </p>
                      </>
                    )}
              ...
              <div className="p-3 bg-emerald-950/60 border border-emerald-500/40 rounded-2xl text-xs text-center font-bold text-emerald-300">
                🏆 Built by Team KHET2TECH for Smart India Hackathon 2026 (Problem Statement: SIH26033)
              </div>
            </div>
          )}
