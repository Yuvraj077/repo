...
export default function WhatIfSimulatorPage() {
  ...
  const farmerFriendlyAdvice = React.useMemo(() => {
    if (dieselDiff > 25 && conditions.ambientTemperatureC > 38) {
      return `Extreme double shock! Diesel is +₹${dieselDiff}/L and heat is ${conditions.ambientTemperatureC}°C. Khet2Tech actively pre-cools at Doaba Cold Hub to prevent a -₹40,000 rot disaster while pooling reefer trucks!`;
    }
    if (dieselDiff > 20) {
      return `Diesel is high (+₹${dieselDiff}/L). Driving far burns money. Notice how Khet2Tech redirects Gurmail to Doaba Cold Hub (just 12 km away) to pool logistics rather than driving solo to distant mandis!`;
    }
    if (conditions.ambientTemperatureC >= 40) {
      return `Extreme heatwave (${conditions.ambientTemperatureC}°C). Open tractors will lose 40% of harvest to sun rot in mandi queues. Khet2Tech locks in pre-cooling to guarantee 0% spoilage.`;
    }
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Click the 3 tabs below to understand how Khet2Tech models real agricultural economics for farmers:
              </p>
            </div>
          </div>
        </div>
        ...
        {activeKnowledgeTab === 'diesel' && (
          <div className="p-5 rounded-2xl bg-slate-950/80 border border-slate-800 space-y-4 animate-in fade-in duration-200">
            ...
            <div className="p-3.5 bg-amber-950/80 border border-amber-500/80 rounded-xl text-xs text-amber-100 font-medium leading-relaxed">
              💡 <strong className="text-amber-300 font-bold">The Bottom Line:</strong> The farmer loses 20 paise per kg (₹1,000 lost on a 5,000 kg lot) purely to expensive diesel! Khet2Tech&apos;s job is to instantly calculate which route loses the least money.
            </div>
          </div>
        )}
          ...
          <div className="p-5 rounded-2xl bg-slate-950 border border-slate-800 space-y-4 animate-in fade-in duration-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
              ...
              <div className="p-3.5 bg-emerald-950/90 border border-emerald-500 rounded-xl space-y-1.5 text-emerald-100">
                <span className="font-bold block text-emerald-300 text-xs">✓ With Khet2Tech Cold Pre-Cooling Hub:</span>
